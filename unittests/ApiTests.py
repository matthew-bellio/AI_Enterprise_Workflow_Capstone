...
class ApiTest(unittest.TestCase):
    ...
    def test_02_predict(self):
        ...
        data = {'country':'all','year':'2018','month':'05','day':'01','mode':'test'}

        j_data = json.dumps(data)
        headers = {'content-type': 'application/json', 'Accept-Charset': 'UTF-8'}

        r = requests.post('http://127.0.0.1:{}/predict'.format(port), data=j_data, headers=headers)
        response = json.loads(r.text)

        self.assertGreaterEqual(response['y_pred'], [1])
